...
function addGamesToPage(games) {
    ...
    for(let i= 0; i< games.length; i++)
    {
        ...
        if (games[i].pledged >= games[i].goal) {  //This is for the bonus feature. Funded games will have different color
            gameCard.classList.add("funded-game");
        ...
        <p><strong>Description:</strong> $${games[i].description.toLocaleString()}</p>`;
...
function filterUnfundedOnly() {
    ...
    const belowGoal = GAMES_JSON.filter((game)=> { return game.pledged < game.goal;});

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(belowGoal);
...
function filterFundedOnly() {
    ...
    const aboveGoal = GAMES_JSON.filter((game)=> { return game.pledged >= game.goal;});

    // use the function we previously created to add unfunded games to the DOM
    addGamesToPage(aboveGoal);
...
const numOfUnfunded = GAMES_JSON.filter((game)=> { return game.pledged < game.goal;}).length;
